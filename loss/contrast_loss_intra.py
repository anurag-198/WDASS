...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y, y_prob): #feat, labels, pred #f_fin, gts_fin, pred_fin, prob_fin
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:   
                ...
                easy_indices = ((this_y_hat == cls_id) & (this_y == cls_id) & (this_prob > 0.96)).nonzero()
    ...
    def _contrastive(self, feats_, labels_, prototypes):
        ...
        #logits_mask = torch.ones_like(mask).scatter_(1, torch.arange(anchor_num * anchor_count).view(-1, 1).cuda(), 0) ### 0 in the diagonal after repeat
        #mask = mask * logits_mask ### not including the diagonal all positives, this not needed with prototypes
    ...
    #def forward(self, feats, labels=None, pred=None): call this loss at the end
    def forward(self, features, gts, pred, prob, prototypes) :
        gts = gts.unsqueeze(1).float().clone()
        pred = pred.unsqueeze(1).float().clone()

        pseudo_label_big = torch.nn.functional.interpolate(pred.clone(), (gts.shape[2], gts.shape[3]), mode='nearest')  
        pseudo_label_big[gts != 255] = gts[gts != 255]  ### to avoid gts from loosing inf with downsampling, all samples easy, use pseudo prob, only for points
        
        pseudo_label_big = torch.nn.functional.interpolate(pseudo_label_big,        
                                                 (features.shape[2], features.shape[3]), mode='nearest')  ### downsampling issue for point

        pseudo_label_big = pseudo_label_big.squeeze(1).long()
        
        pred = pred.squeeze(1).long()
        gts = pseudo_label_big
        
        f_fin = features
        #f_fin = F.normalize(features, dim=1)  ### in a unit square, now doing in model itself
        
        gts = gts.squeeze(1).long()

        assert gts.shape[-1] == f_fin.shape[-1], '{} {}'.format(gts.shape, f_fin.shape)

        batch_size = f_fin.shape[0]

        gts_fin = gts.contiguous().view(batch_size, -1)
        pred_fin = pred.contiguous().view(batch_size, -1)
        prob_fin = prob.contiguous().view(batch_size, -1)

        f_fin = f_fin.permute(0, 2, 3, 1)
        f_fin = f_fin.contiguous().view(f_fin.shape[0], -1, f_fin.shape[-1])

        feats_, labels_ = self._hard_anchor_sampling(f_fin, gts_fin, pred_fin, prob_fin)
        
        if feats_ is None or feats_.shape[0] == 0 :
            return torch.tensor(0.0).cuda()

        loss = self._contrastive(feats_, labels_, prototypes)
        
        return loss

class ContrastCELoss_intra(nn.Module, ABC):
    def __init__(self, weak_label=None):
        super(ContrastCELoss_intra, self).__init__()
        ignore_index = 255
        self.contrast_criterion = PixelContrastLoss(weak_label)

    def forward(self,features, gts, pred, prob, prototypes): ### features, gt (features_s, gts_s, features_u, pseudo_label, gts_u)
        loss_contrast = self.contrast_criterion(features, gts, pred, prob, prototypes)
